...
public class DataServiceController {
    ...
    @GetMapping public ApiResponse<List<DataServiceDefinition>> definitions() { return ApiResponse.success(service.definitions()); }
    ...
    public ApiResponse<DataServiceDefinition> update(@PathVariable Long id, @Valid @RequestBody DataServiceDTO.DefinitionRequest request) {
        return ApiResponse.success("数据服务已更新", service.updateDefinition(id, request));
    }
    ...
    public ApiResponse<DataServiceDefinition> publish(@PathVariable Long id, @RequestParam(defaultValue="true") boolean published) {
        return ApiResponse.success(published ? "数据服务已发布" : "数据服务已下线", service.publish(id, published));
    }
    @DeleteMapping("/{id}") @PreAuthorize("hasAuthority('data-service:manage')")
    public ApiResponse<Void> delete(@PathVariable Long id) { service.deleteDefinition(id); return ApiResponse.success("数据服务已删除", null); }

    @GetMapping("/apps") public ApiResponse<List<DataServiceApp>> apps() { return ApiResponse.success(service.apps()); }
    ...
    public ApiResponse<DataServiceDTO.AppCredential> rotate(@PathVariable Long appId) { return ApiResponse.success("密钥已轮换", service.rotateSecret(appId)); }
    @PostMapping("/apps/{appId}/toggle") @PreAuthorize("hasAuthority('data-service:manage')")
    public ApiResponse<DataServiceApp> toggle(@PathVariable Long appId) { return ApiResponse.success("应用状态已更新", service.toggleApp(appId)); }
    @GetMapping("/apps/{appId}/grants") public ApiResponse<List<DataServiceGrant>> grants(@PathVariable Long appId) { return ApiResponse.success(service.grants(appId)); }
    ...
    public ApiResponse<Void> revoke(@PathVariable Long appId, @PathVariable Long serviceId) { service.revoke(appId, serviceId); return ApiResponse.success("服务授权已移除", null); }
    @GetMapping("/logs") public ApiResponse<List<DataServiceInvocationLog>> logs(@RequestParam(defaultValue="100") int limit) { return ApiResponse.success(service.logs(limit)); }
}

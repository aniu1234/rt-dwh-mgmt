...
| 对象 | 关键属性 | 生命周期关注点 |
|---|---|---|
| 数据源 | 类型、连接、凭证、额外配置 | 创建、测试、探测、停用、密钥轮换 |
| 同步／开发任务 | 类型、SQL、映射、并行度、Checkpoint | draft、submitting、running、paused、failed、finished |
| 任务版本 | 版本号、SQL、配置快照、发布人 | 发布、比较、回滚为 draft |
| 运行实例 | 业务日期、批次、参数、执行器、租约、重试 | waiting、queued、running、success、failed、cancelled |
...
场景编码 scenarioCode
    ├── table_realtime_sync ─────┐
    ├── database_realtime_sync ──┼──> cdc_sync 执行适配器
    ├── sql_transform ───────────────> etl 执行适配器
    └── materialized_table ──────────> materialized 执行适配器
```
...
| Flink SQL 加工 | 已开放 | `etl` | 选择输入输出数据源并填写 SQL |
| 物化表任务 | 已开放 | `materialized` | 填写物化表定义并持续维护结果 |
| Kafka 实时入湖 | 已预留 | 待复用或新增 | 增加 Kafka 数据源、Topic 与 Schema 配置面板 |
| 文件批量入湖 | 已预留 | 待新增 | 增加对象存储、目录、格式与去重配置面板 |
| 定时数据产出 | 已预留 | `etl` + 调度 | 串联周期、依赖、补数和产出登记 |
...
1. 创建 ETL／物化任务并维护 SQL；
2. 配置上下游依赖，平台执行 DAG 环路检查；
3. 发布不可变任务版本，配置 Cron、时区、业务日期偏移和参数；
